...
export default function Follows({ currentUserId, profileUserId }: FollowsProps) {
  ...
  const displayedUsers: FollowUser[] = activeTab === "followers" ? followers : following;
  const isLoading = activeTab === "followers" ? loadingFollowers : loadingFollowing;

  return (
    <main className="w-[50%] border-r border-gray-800 min-h-screen">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 min-w-0">
                  <img
                    src={user.photo_profile || `https://i.pravatar.cc/40?u=${user.id}`}
                    className="w-10 h-10 rounded-full object-cover border border-gray-700"
                  />

                  <div className="min-w-0">
                    <span onClick={() => Navigate(`/profile/${user.id}`)} className="text-white text-sm font-semibold cursor-pointer truncate">
                      {user.full_name}
                    </span>
